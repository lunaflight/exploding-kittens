...
let fill_uncensored_alert_template t ~player_name ~possessive_pronoun =
  ...
  | Stole_randomly (card, target) ->
    [%string
      "%{player_name#Player_name} stole a(n) %{card#Card} from \
       %{target#Player_name}."]
;;
...
let to_specialised_alert t ~player_name =
  ...
  | Stole_randomly (card, target) ->
    ( target
    , [%string "%{player_name#Player_name} stole a(n) %{card#Card} from you."]
    )
    |> Some
;;
...
let to_censored_alert t ~player_name =
  ...
  | Stole_randomly (_card, target) ->
    [%string
      "%{player_name#Player_name} stole a random card from \
       %{target#Player_name}."]
;;
